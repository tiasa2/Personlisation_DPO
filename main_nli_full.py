...
def classify_reasoning_against_axioms(reasoning, axioms):
    predicted_labels = []
    prompts = []
    for axiom in axioms: 
        prompt = f"""You are a logical reasoning validator. Given the axiom and reasoning:
        1. Axiom : {axiom}
        2. Reasoning step {reasoning}

        Your task is to determine the logical relationship:

        - entailment: The reasoning logically follows from the axiom (it must be true)
        - contradiction: The reasoning contradicts the axiom (it cannot be true)
        - neutral: The reasoning is not determined by the axiom (it might or might not be true)
        Just respond with one of the three labels: 'entailment', 'contradiction', or 'neutral'."""
...
def faithfulness_check(axioms, reasoning_chain):
    ...
    #     if reason != '':
    #         output_premise, output_predicted_labes, output = classify_reasoning_against_axioms(reason, axioms.split('\n')[1:])
    #         print('reason', output)
    ...
    output_premise, output_predicted_labels, output = classify_reasoning_against_axioms(reasoning_chain, axioms.split('\n')[1:])
    print('--------------------reasoning chain end-------------------')
    # if counter == 0:
    #     return (output_premises, output_predicted_labels, 0)
    # print('faithfulness', output_sum/counter)
    return (output_premise, output_predicted_labels, output)
...
json.dump(final_dicts, open("combined_llama70B_NLI_full_0_to_25.json", "w"), indent=4)
json.dump(final_output, open("combined_llama70B_NLI_full_outputs_0_to_25.json", "w"), indent=4)
